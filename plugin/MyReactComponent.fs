...
type MyReactComponentAttribute(?exportDefault: bool, ?import: string, ?from:string) =
    inherit MemberDeclarationPluginAttribute()
    override _.FableMinimumVersion = "3.0"
    new() = MyReactComponentAttribute(exportDefault=false)
    new(exportDefault: bool) = MyReactComponentAttribute(exportDefault=exportDefault,?import=None, ?from=None)
    new(import: string, from: string) = MyReactComponentAttribute(exportDefault=false,import=import, from=from)
        ...
        | Fable.Call(callee, info, typeInfo, range) ->
            ...
            if info.Args.Length = 1 && AstUtils.isRecord compiler info.Args.[0].Type then
                ...
                let propsObj =
                    ...
                    let typ =
                        kvs
                        |> List.map (fun (name, expr) -> name, expr.Type)
                        |> AstUtils.makeAnonRecordType
                    AstUtils.objExprTyped typ kvs
        ...
        if decl.Info.IsValue || decl.Info.IsGetter || decl.Info.IsSetter then
            ...
            if decl.Args.Length = 1 && AstUtils.isRecord compiler decl.Args.[0].Type then
                ...
                let args =
                    decl.Args
                    |> List.map (fun arg ->
                        let name = if arg.DisplayName = "key" then "$key" else arg.DisplayName
                        name, arg.Type)
                let propsArg =
